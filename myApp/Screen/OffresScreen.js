...
// API Service
import { offerService } from '../services/offresApi.js'; 
...
const OffreScreen = ({ navigation }) => {
  ...
  const fetchOffres = async () => {
    try {
      ...
      const apiOffers = response.offers || response.data || response;
      
      if (Array.isArray(apiOffers)) {
        setOffres(apiOffers);
        console.log("[OffreScreen] ✅ Loaded", apiOffers.length, "offers");
      } else {
        console.log("[OffreScreen] ⚠️ Response is not an array");
        setOffres([]);
      }
    } catch (e) {
  ...
  const secteurs = [
    'Toutes les offres', 
    ...new Set(offres.map((offer) => offer.secteur).filter(Boolean))
  ];
  ...
  const getFieldIcon = (field) => {
    const icons = { 
      ...
      'Telecommunication': 'cellphone',
      'Télécommunications': 'cellphone',
      'Finance': 'cash',
      'Healthcare': 'hospital',
      'Santé': 'hospital',
      'Education': 'school',
          ...
          <View style={styles.fullDetailContainer}>
            ...
            <ScrollView bounces={false}>
              ...
              <View style={styles.detailBody}>
                ...
                {selectedOffre.website && selectedOffre.website !== "#" && (
                  <TouchableOpacity 
                    style={styles.applyButton} 
                    onPress={() => Linking.openURL(selectedOffre.website)}
                  >

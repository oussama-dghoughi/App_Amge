...
const initialTimelineData = [
  { id: 1, day: 7, time: '9h30', title: "Le Marché de l'Emploi en Afrique", category: 'Conférence', description: "Discussion sur les opportunités d'emploi et les tendances du marché.", isChecked: true },
  { id: 2, day: 7, time: '10h30', title: "Rédiger un CV Impactant", category: 'Atelier', description: "Un atelier pour améliorer vos CV.", isChecked: false },
  { id: 3, day: 7, time: '17h00', title: "Clôture & Remerciements", category: 'Conférence', description: "Remerciements et clôture du Forum.", isChecked: false },
  { id: 4, day: 7, time: '19h00', title: "Cérémonie de clôture & networking", category: 'Cérémonie', description: "Cérémonie de clôture du Forum & buffet dinatoire.", isChecked: false },
];
...
const CalendarWidget = ({ selectedDate }) => (
  <View style={styles.calendarContainer}>
    <LinearGradient colors={[COLORS.gradientStart, COLORS.gradientEnd]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.dateBannerGradient}>
      <Text style={styles.dateBannerText}>Dimanche {selectedDate} Juin 2026</Text>
    </LinearGradient>
  </View>
);
  ...
  return (
    <View style={styles.tlItemContainer}>
      <View style={styles.tlTimeContainer}>
        <GradientText text={time} style={styles.tlTimeText} />
      </View>
      ...
      <View style={styles.tlContentContainer}>
        ...
        <LinearGradient colors={[COLORS.gradientStart, COLORS.gradientEnd]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.tlBadgeContainer}>
          ...
          <View style={{ paddingHorizontal: 20 }}>
            {currentEvents.length === 0 ? (
              ...
              currentEvents.map((item, index) => (
                <TimelineItem key={item.id} data={item} isLast={index === currentEvents.length - 1} onToggle={toggleEventCheck} />
              ))
            )}
          </View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  safeAreaHeader: { backgroundColor: '#fff' },
  ...
  dateBannerGradient: { borderRadius: 30, paddingVertical: 12, paddingHorizontal: 25, alignSelf: 'center', marginBottom: 20 },
  dateBannerText: { fontSize: 26, fontWeight: 'bold', color: COLORS.black },
  bodyBackground: { flex: 1 },
  ...
  tlTimeText: { fontSize: 15, fontWeight: 'bold' },
  ...
  tlBadgeText: { color: 'black', fontSize: 12, fontWeight: 'bold' },
  ...
  bottomNavContainer: { position: 'absolute', bottom: 0, left: 0, right: 0, height: 70, backgroundColor: '#fff', borderTopWidth: 1, borderTopColor: '#EEE' },
});
